...
class GeometryTypeValidator(FeatureValidator):
    ...
    @classmethod
    def validate(cls, run_id: int, validation_code: str, severity: str, feature_class: QgsVectorLayer, expected_geometry_type: QgsWkbTypes.GeometryType) -> list[ValidationResult]:
        ...
        Args:
            ...
            expected_geometry_type (QgsWkbTypes.GeometryType): The expected geometrytype.
        ...
        if not feature_class.wkbType() == expected_geometry_type:
            log_message = f"The geometrytype of layer {feature_class.name()} is {QgsWkbTypes.displayString(feature_class.wkbType())} but is expected to be {QgsWkbTypes.displayString(expected_geometry_type)}."
        ...
        for feature in feature_class.getFeatures():
            ...
            if not feature_geom_type == expected_geometry_type:
                message = f"Feature with objectid '{feature['objectid']}' has geometrytype {QgsWkbTypes.displayString(feature_geom_type)} but is expected to be of type {QgsWkbTypes.displayString(expected_geometry_type)}."
